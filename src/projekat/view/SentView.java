...
public class SentView extends VBox implements Runnable{
	...
	public void run() {
		...
		while(true) {
			messages.clear();
			try {
				if (Context.context.getLoggedUser().isPresent()) {
					List<Mail> allMessages = (List<Mail>) serializer.deserializeMessage(new File("messages.bin"));
					for (Mail m : allMessages) {
						if (m.getSender().getUsername().equals(Context.context.getLoggedUser().get().getUsername())) {
							messages.add(m);
						}
					}
					tv.getItems().clear();
	...
	public SentView() {
		...
		t.start();
		setSpacing(10);
		this.getStylesheets().add(getClass().getResource("inboxstyle.css").toExternalForm());
		TableColumn<Mail,User> recipient = new TableColumn<>("To:");
		...
		tv.getColumns().addAll(recipient,subject,text);
		...
		tf.textProperty().addListener(new ChangeListener<String>() {
			...
			public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
				...
				try {
					...
					for(Mail m:messages) {
						if(m.getText().contains(tf.getText()) && m.getSender().getUsername().equals(Context.context.getLoggedUser().get().getUsername()))
							tv.getItems().add(m);
					}
				} catch (Exception e) {
					// TODO Auto-generated catch block
